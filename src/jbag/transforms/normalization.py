...
class MinMaxNormalization(Transform):
    def __init__(self, keys, lower_bound_percentile: float | None = None, upper_bound_percentile: float | None = None):
        """
    ...
    def _call_fun(self, data):
        for key in self.keys:
            image = data[key]
            min_value = np.min(image) if self.lower_bound_percentile is None else np.percentile(image,
                                                                                                self.lower_bound_percentile)
            max_value = np.max(image) if self.upper_bound_percentile is None else np.percentile(image,
                                                                                                self.upper_bound_percentile)
