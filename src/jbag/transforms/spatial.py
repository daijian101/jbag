...
class SpatialTransform(RandomTransform):
    def __init__(self, keys,
                 apply_probability,
                 patch_size=None,
                 patch_center_dist_from_border: int | list[int] | tuple[int, ...] = 0,
                 random_crop: bool = False,
                 interpolation_modes: str | list[str] | tuple[str, ...] = 'nearest',
                 p_rotation: float = 0.0,
                 rotation_angle_range: float | list[float] | tuple[float, float] = (0, 2 * np.pi),
                 p_scaling: float = 0.0,
                 scaling_range: float | list[float] | tuple[float, float] = (0.7, 1.3),
                 p_synchronize_scaling_across_axes: float = 1,
                 p_elastic_deform: float = 0.0,
                 elastic_deform_scale: float | list[float] | tuple[float, float] = (0, 0.2),
                 elastic_deform_magnitude: float | list[float] | tuple[float, float] = (0, 0.2),
                 ):
        """
